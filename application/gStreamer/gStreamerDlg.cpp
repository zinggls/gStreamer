...
UINT CgStreamerDlg::XferBulkOut(LPVOID pParam)
{
	CgStreamerDlg *pDlg = (CgStreamerDlg*)pParam;
	ASSERT(pDlg);
	CCyUSBEndPoint *pEndPt = pDlg->m_pEndPt;
	...
	ASSERT(pEndPt->bIn == FALSE);

	ULONG len = pEndPt->MaxPktSize * pDlg->m_nPPX;
	pEndPt->SetXferSize(len);
...
UINT CgStreamerDlg::XferBulkIn(LPVOID pParam)
{
	CgStreamerDlg *pDlg = (CgStreamerDlg*)pParam;
	ASSERT(pDlg);
	CCyUSBEndPoint *pEndPt = pDlg->m_pEndPt;
